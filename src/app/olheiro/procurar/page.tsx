...
export default function ProcurarTalentosPage() {
  ...
  const aplicarFiltro = () => {
    const filtrados = jogadores.filter((j) => {
      ...
      const pernaOk = filtros.pernaDominante ? j.pernaDominante === filtros.pernaDominante : true;
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-10 pt-28">
        ...
        {planoPremium && (
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
              ...
              <select value={filtros.pernaDominante} onChange={(e) => setFiltros({ ...filtros, pernaDominante: e.target.value })} className="input bg-white border border-gray-300 rounded-md px-3 py-2 text-sm text-[#3a5550] focus:outline-none focus:ring-2 focus:ring-[#f4ae31]">
        ...
        <div className="mb-10 flex flex-wrap gap-4 justify-center">
          ...
          <button onClick={() => {
            setFiltros({ estado: '', cidade: '', posicao: '', pernaDominante: '', idadeMin: '', idadeMax: '', alturaMin: '', alturaMax: '', temVideo: '' });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {resultados.map((jogador) => (
            <div key={jogador.id} className="bg-white border border-gray-200 p-6 rounded-xl shadow-sm text-left text-[#3a5550] hover:shadow-md transition">
              ...
              <p><strong>Perna:</strong> {jogador.pernaDominante}</p>
